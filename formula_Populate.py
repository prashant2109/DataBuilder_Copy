...
class Render():
    ...
    def __get_company_ids(self, company_ids, first=''):
        ...
        sql = """ SELECT row_id, project_id, company_name, doc_id, page_no, grid_id FROM formula_populate_info WHERE ((populate_status in ('N') AND queue_status in ('N')) or (populate_status in ('Y', 'E') AND queue_status in ('Q'))) AND project_id='80' LIMIT %s; """%(self.core_count) 
        cur.execute(sql)
        t_data = cur.fetchall()
        cur.close()
        conn.close()
        row_dct   = {}
        for row in t_data:      
            row_id, project_id, company_name, doc_id, page_no, grid_id = row
            company_name = en_obj.convert(company_name)
            if isinstance(company_name, unicode):
                company_name = company_name.encode('utf-8')
            row_id, project_id, doc_id, page_no, grid_id = map(str, (row_id, project_id, doc_id, page_no, grid_id))
